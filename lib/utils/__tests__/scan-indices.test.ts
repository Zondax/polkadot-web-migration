...
describe('scan-indices utilities', () => {
  describe('Type guards', () => {
    describe('isSingleScan', () => {
      it('should return true for single scan type', () => {
        expect(isSingleScan(ScanTypeEnum.SINGLE)).toBe(true)
      })

      it('should return false for range scan type', () => {
        expect(isSingleScan(ScanTypeEnum.RANGE)).toBe(false)
      })
    })

    describe('isRangeScan', () => {
      it('should return true for range scan type', () => {
        expect(isRangeScan(ScanTypeEnum.RANGE)).toBe(true)
      })

      it('should return false for single scan type', () => {
        expect(isRangeScan(ScanTypeEnum.SINGLE)).toBe(false)
      })
    })

    describe('Config type guards', () => {
      it('should identify single config correctly', () => {
        const singleConfig = { type: ScanTypeEnum.SINGLE as const, value: 5 }
        const rangeConfig = { type: ScanTypeEnum.RANGE as const, start: 1, end: 5 }
  ...
  describe('parseIndexConfig', () => {
    describe('Single scan type', () => {
      it('should parse valid single index', () => {
        const config = parseIndexConfig(ScanTypeEnum.SINGLE, '5', '0', '10')
        expect(config).toEqual({ type: ScanTypeEnum.SINGLE, value: 5 })
      })

      it('should parse zero index', () => {
        const config = parseIndexConfig(ScanTypeEnum.SINGLE, '0', '1', '5')
        expect(config).toEqual({ type: ScanTypeEnum.SINGLE, value: 0 })
      })

      it('should return null for invalid single index', () => {
        expect(parseIndexConfig(ScanTypeEnum.SINGLE, 'invalid', '0', '10')).toBeNull()
        expect(parseIndexConfig(ScanTypeEnum.SINGLE, '-1', '0', '10')).toBeNull()
        expect(parseIndexConfig(ScanTypeEnum.SINGLE, '', '0', '10')).toBeNull()
      })
    })

    describe('Range scan type', () => {
      it('should parse valid range', () => {
        const config = parseIndexConfig(ScanTypeEnum.RANGE, '0', '1', '5')
        expect(config).toEqual({ type: ScanTypeEnum.RANGE, start: 1, end: 5 })
      })

      it('should parse single-value range', () => {
        const config = parseIndexConfig(ScanTypeEnum.RANGE, '0', '3', '3')
        expect(config).toEqual({ type: ScanTypeEnum.RANGE, start: 3, end: 3 })
      })

      it('should return null for invalid range values', () => {
        expect(parseIndexConfig(ScanTypeEnum.RANGE, '0', 'invalid', '5')).toBeNull()
        expect(parseIndexConfig(ScanTypeEnum.RANGE, '0', '1', 'invalid')).toBeNull()
        expect(parseIndexConfig(ScanTypeEnum.RANGE, '0', '5', '1')).toBeNull() // end < start
        expect(parseIndexConfig(ScanTypeEnum.RANGE, '0', '-1', '5')).toBeNull() // negative start
      })
    })
  })
  ...
  describe('validateIndexConfig', () => {
    describe('Single scan validation', () => {
      it('should validate valid single index', () => {
        const result = validateIndexConfig(ScanTypeEnum.SINGLE, '5', '0', '10')
        expect(result).toEqual({ isValid: true })
      })

      it('should validate zero index', () => {
        const result = validateIndexConfig(ScanTypeEnum.SINGLE, '0', '1', '5')
        expect(result).toEqual({ isValid: true })
      })

      it('should reject invalid single index', () => {
        const result = validateIndexConfig(ScanTypeEnum.SINGLE, 'invalid', '0', '10')
        expect(result.isValid).toBe(false)
        expect(result.error).toBe('Index must be a number')
      })

      it('should reject negative single index', () => {
        const result = validateIndexConfig(ScanTypeEnum.SINGLE, '-1', '0', '10')
        expect(result.isValid).toBe(false)
    ...
    describe('Range scan validation', () => {
      it('should validate valid range', () => {
        const result = validateIndexConfig(ScanTypeEnum.RANGE, '0', '1', '5')
        expect(result).toEqual({ isValid: true })
      })

      it('should validate single-value range', () => {
        const result = validateIndexConfig(ScanTypeEnum.RANGE, '0', '3', '3')
        expect(result).toEqual({ isValid: true })
      })

      it('should reject invalid range values', () => {
        let result = validateIndexConfig(ScanTypeEnum.RANGE, '0', 'invalid', '5')
        expect(result.isValid).toBe(false)
        expect(result.error).toBe('Range values must be numbers')

        result = validateIndexConfig(ScanTypeEnum.RANGE, '0', '1', 'invalid')
        expect(result.isValid).toBe(false)
        expect(result.error).toBe('Range values must be numbers')
      })
      ...
      it('should reject end < start', () => {
        const result = validateIndexConfig(ScanTypeEnum.RANGE, '0', '5', '1')
        expect(result.isValid).toBe(false)
        expect(result.error).toBe('End index must be greater than or equal to start')
      })

      it('should reject range that exceeds limit', () => {
        const result = validateIndexConfig(ScanTypeEnum.RANGE, '0', '0', SCAN_LIMITS.MAX_RANGE.toString())
        expect(result.isValid).toBe(false)
  ...
  describe('getIndexCount', () => {
    it('should return 1 for single scan', () => {
      expect(getIndexCount(ScanTypeEnum.SINGLE, '5', '0')).toBe(1)
    })

    it('should calculate range count correctly', () => {
      expect(getIndexCount(ScanTypeEnum.RANGE, '0', '1')).toBe(2) // 0,1
      expect(getIndexCount(ScanTypeEnum.RANGE, '0', '0')).toBe(1) // just 0
      expect(getIndexCount(ScanTypeEnum.RANGE, '0', '3')).toBe(4) // 0,1,2,3
      expect(getIndexCount(ScanTypeEnum.RANGE, '0', '5')).toBe(6) // 0,1,2,3,4,5
    })

    it('should handle invalid values gracefully', () => {
      expect(getIndexCount(ScanTypeEnum.RANGE, 'invalid', 'invalid')).toBe(1) // invalid values default to 0, so 0-0 = 1
      expect(getIndexCount(ScanTypeEnum.RANGE, '0', 'invalid')).toBe(1) // invalid end defaults to 0, so 0-0 = 1
    })
  })

  describe('getIndexCountFromConfig', () => {
    it('should return 1 for single config', () => {
      const config = { type: ScanTypeEnum.SINGLE as const, value: 5 }
      expect(getIndexCountFromConfig(config)).toBe(1)
    })

    it('should calculate range count from config', () => {
      const config = { type: ScanTypeEnum.RANGE as const, start: 1, end: 5 }
  ...
  describe('formatIndexDisplay', () => {
    it('should format single index', () => {
      expect(formatIndexDisplay(ScanTypeEnum.SINGLE, '5', '0', '10')).toBe('5')
      expect(formatIndexDisplay(ScanTypeEnum.SINGLE, '0', '1', '5')).toBe('0')
    })

    it('should format range display', () => {
      expect(formatIndexDisplay(ScanTypeEnum.RANGE, '0', '1', '5')).toBe('{1...5}')
      expect(formatIndexDisplay(ScanTypeEnum.RANGE, '0', '3', '3')).toBe('3') // same start/end
    })

    it('should handle invalid values', () => {
      expect(formatIndexDisplay(ScanTypeEnum.SINGLE, 'invalid', '0', '10')).toBe('0')
      expect(formatIndexDisplay(ScanTypeEnum.RANGE, '0', 'invalid', '5')).toBe('{0...5}')
    })
  })
  ...
  describe('formatIndexDisplayFromConfig', () => {
    ...
    it('should format range config', () => {
      ...
      const sameConfig = { type: ScanTypeEnum.RANGE as const, start: 3, end: 3 }
  ...
  describe('getRangeValue', () => {
    ...
    it('should get start value', () => {
      expect(getRangeValue(range, RangeFieldEnum.START)).toBe('1')
    })

    it('should get end value', () => {
      expect(getRangeValue(range, RangeFieldEnum.END)).toBe('5')
    })
  })
  ...
  describe('updateRangeValue', () => {
    ...
    it('should update start value', () => {
      const updated = updateRangeValue(range, RangeFieldEnum.START, '2')
      expect(updated).toEqual({ start: '2', end: '5' })
    })

    it('should update end value', () => {
      const updated = updateRangeValue(range, RangeFieldEnum.END, '7')
      expect(updated).toEqual({ start: '1', end: '7' })
    })

    it('should not mutate original range', () => {
      updateRangeValue(range, RangeFieldEnum.START, '10')
  ...
  describe('generateIndicesArray', () => {
    it('should generate single index array', () => {
      const result = generateIndicesArray(ScanTypeEnum.SINGLE, 5)
      expect(result).toEqual([5])
    })

    it('should generate range index array', () => {
      const result = generateIndicesArray(ScanTypeEnum.RANGE, undefined, 1, 5)
      expect(result).toEqual([1, 2, 3, 4, 5])
    })

    it('should generate single-value range', () => {
      const result = generateIndicesArray(ScanTypeEnum.RANGE, undefined, 3, 3)
      expect(result).toEqual([3])
    })

    it('should return empty array for invalid inputs', () => {
      expect(generateIndicesArray(ScanTypeEnum.SINGLE)).toEqual([])
      expect(generateIndicesArray(ScanTypeEnum.RANGE, undefined, 1)).toEqual([])
      expect(generateIndicesArray(ScanTypeEnum.RANGE, undefined, undefined, 5)).toEqual([])
    })
  })
})
