...
function GovernanceRefundForm({
  ...
  const getDepositBadge = (deposit: GovernanceDeposit) => {
    if (deposit.canRefund) {
      return <Badge className="bg-green-100 text-green-800 border-green-300 text-xs">Refundable</Badge>
    }
    if (deposit.referendumStatus === 'ongoing') {
      return <Badge className="bg-blue-100 text-blue-800 border-blue-300 text-xs">Ongoing</Badge>
    }
    return <Badge className="bg-gray-100 text-gray-800 border-gray-300 text-xs">Not Refundable</Badge>
  }
        ...
        <div className="flex flex-col gap-1.5 max-h-40 overflow-y-auto">
          {deposits.map(deposit => (
            <div
              ...
              className={`flex items-center justify-between text-xs p-2 rounded-lg ${getDepositBadgeStyle(deposit)}`}
            >
